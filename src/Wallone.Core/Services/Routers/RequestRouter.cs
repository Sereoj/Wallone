...
namespace Wallone.Core.Services.Routers
{
    public class RequestRouter<T> : Router
    {
        ...
        public static async Task<T> GetWithTokenAsync(string method, string page, List<Parameter> parameters)
        {
    ...
    public class RequestRouter<T, T2> : Router
    {
        ...
        public static async Task<T> PostWithTokenAsync(string method, T2 model, List<Parameter> parameters = null)
        {
            var token = new SettingsBuilder(SettingsRepository.Get())
                .ItemBuilder()
                .GetToken();
